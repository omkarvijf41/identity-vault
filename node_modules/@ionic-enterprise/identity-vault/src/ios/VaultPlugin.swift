...
class VaultPlugin: CDVPlugin {
    ...
    private var errorCommandCallback: CDVInvokedUrlCommand?
    ...
    @objc(onError:) func onError(_ command: CDVInvokedUrlCommand) {
        self.errorCommandCallback = command;
    }

    @objc(onLock:) func onLock(_ command: CDVInvokedUrlCommand) {
        do {
            let config = try self.getConfig(command)
            try vaultManager.onLock(config, {
    ...
    private func clearIfFirstRun(config: IdentityVaultConfig) -> Void {
    ...
    private func sendErrorResult(_ command: CDVInvokedUrlCommand, _ error: Error) {
        ...
        commandDelegate.send(result, callbackId: command.callbackId)
        if self.errorCommandCallback != nil {
            commandDelegate.send(result, callbackId: self.errorCommandCallback?.callbackId)
        }
    }
}
