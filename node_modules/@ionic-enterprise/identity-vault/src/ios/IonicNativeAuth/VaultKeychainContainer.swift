...
public class VaultKeychainContainer {
    ...
    private var _passcode: String? {
        set {
            ...
            _credential.passcode = try? encryptPasscode(passcode, true)
        }
    ...
    private func encryptPasscode(_ passcode: String, _ setNew: Bool = false) throws -> String {
    ...
    private func ensurePasscodeInMemory() throws {
        guard _passcode != nil else {
            throw VaultError.missingPasscode(false)
        }
    }
    ...
    private func doPinDialog(_ title: String = "SetPIN", _ completion: @escaping (String?) -> Void) {
        pinDialog.displayPasscodePrompt(alertTitle: NSLocalizedString(title, tableName: nil, comment: ""),
                                        dispCancel: true, completion: {(pinOkay, pin) in
                                            if pinOkay {
                                                completion(pin)
                                            } else {
                                                completion(nil)
    ...
    public init(_ vaultConfig: VaultConfig) throws {
    ...
    private func storeConfig() throws {
        do {
            ...
            throw VaultError.unhandledError(error.localizedDescription)
        }
    }
    ...
    public func getKeys() throws -> [String]? {
        ...
        if let lazyMapCollection = credential?.keys {
            let stringArray = [String](lazyMapCollection)
            return stringArray
        }
        return []
    }
    ...
    private func getCredential(_ withPasscode: String? = nil) throws -> [String: Any]? {
        ...
        do {
            ...
            if secureStorageModeEnabled {
                ...
                keychainData = try passwordItem.readPassword(getReadKey(usingPasscode), withPasscode, self.vaultConfig.iosPromptText)
            }
    ...
    private func authByPinDialog(_ completion: ((Error?) -> Void)? ) {
        doPinDialog("VerifyPIN") { (pin) in
            ...
            do {
                _ = try self.getCredential(self.encryptPasscode(pin!))
                completion!(nil)
            } catch {
                return completion!(error)
            }
    ...
    public func unlock(_ usePasscode: Bool = false, _ withPasscode: String? = nil, _ completion: @escaping (Error?) -> Void) throws {
        ...
        if usePasscode {
            if withPasscode != nil {
                _ = try self.getCredential(self.encryptPasscode(withPasscode!))
                return completion(nil)
            }
            return self.authByPinDialog(completion)
        }
        // use biometrics if it's enabled & no passcode provided
        // run async on in background so willResignActive handler fires correctly
        DispatchQueue.global().async {
            do {
                _ = try self.getCredential()
                completion(nil)
            } catch {
                return completion(error)
            }
    ...
    public func clearConfig() throws {
        config = VaultKeychainContainer.createNewConfig(descriptor)
        try storeConfig()
    }

    public func clearIfFirstRun() {
    ...
    public func migrateVault() throws {
        //        if config.version < 3 {
        //            // TODO: migration code
        //        }
