...
struct AES256Crypter {
    ...
    public func crypt(input: Data, operation: CCOperation) throws -> Data {
        var outLength = Int(0)
        var outBytes = [UInt8](repeating: 0, count: input.count + kCCBlockSizeAES128)
        var status: CCCryptorStatus = CCCryptorStatus(kCCSuccess)
        input.withUnsafeBytes { (encryptedBytes: UnsafeRawBufferPointer!) -> Void in
        ...
        password.withUnsafeBytes { (passwordBytes: UnsafeRawBufferPointer!) in
            salt.withUnsafeBytes { (saltBytes: UnsafeRawBufferPointer!) in
                status = CCKeyDerivationPBKDF(CCPBKDFAlgorithm(kCCPBKDF2),                  // algorithm
                                              passwordBytes.baseAddress?.assumingMemoryBound(to: Int8.self),         // password
...
extension AES256Crypter: Crypter {
    ...
    func encrypt(_ digest: Data) throws -> Data {
        return try self.crypt(input: digest, operation: CCOperation(kCCEncrypt))
    }

    func decrypt(_ encrypted: Data) throws -> Data {
        return try self.crypt(input: encrypted, operation: CCOperation(kCCDecrypt))
    }
