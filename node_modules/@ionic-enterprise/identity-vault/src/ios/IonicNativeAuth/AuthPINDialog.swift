...
class AuthPINDialog: NSObject {
    ...
    func showAlertView(
        ...
        for title in actionTitles {
            // add action.
            let action = UIAlertAction(title: title, style: .default, handler: {[weak alert] (_: UIAlertAction) -> Void in
                if let acts = actions {
                    if acts.count >= actionTitles.count {
                        acts[actionTitles.firstIndex(of: title)!](alert!)
                    }
                }
    ...
    public func displayPasscodePrompt(alertTitle: String, dispCancel: Bool, completion: @escaping (Bool, String) -> Void) {
