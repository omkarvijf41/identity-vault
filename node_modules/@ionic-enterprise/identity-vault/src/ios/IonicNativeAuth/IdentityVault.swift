...
public class IdentityVault {
    ...
    public init(_ setupOptions: [String: Any], _ descriptor: VaultDescriptor, eventHandler: @escaping EventHandler) throws {
    ...
    private func obscureScreen() {
        ...
        if #available(iOS 13, *) {
            window = UIApplication.shared.connectedScenes
                .filter({$0.activationState == .foregroundActive || $0.activationState == .foregroundInactive}).compactMap({$0 as? UIWindowScene}).first?.windows.filter({$0.isKeyWindow}).first
        } else {
