...
class IonicNativeAuthPlugin: CDVPlugin {
    ...
    @objc func setup(_ command: CDVInvokedUrlCommand) {
        ...
        if authVault == nil {
            do {
                authVault = try IdentityVault(setupOptions, descriptor, eventHandler: sendEvents)
    ...
    @objc func setHideScreenOnBackground(_ command: CDVInvokedUrlCommand) {
        guard let vault = ensureVault(command) else { return }
        guard let hideScreen = command.arguments[1] as? Bool else {
            return sendResult(command, VaultError.invalidArguments("enabled arg missing or not a boolean"))
        }

        do {
            try vault.setHideScreenOnBackground(hideScreen)
