    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        if (Device.getInstance().isHideScreenOnBackgroundEnabled()) {
            try {
                ...
                Drawable splash = getResources().getDrawable(resourceId);
                setTheme(themeId);
                getWindow().getDecorView().setBackground(splash);
            } catch (Exception e) {
                e.printStackTrace();
                final int themeId = getResources().getIdentifier("Blurred", "style", getPackageName());
                setTheme(themeId);
            }
        }
    ...
    private void authenticate() {
        try {
                ...
                if (masterKeyCryptoData != null) {
                    cipher =
                        BiometricCipherHelper.getCypherForDecryption(
                            this.vault.config.key,
                            vault.config.deviceSecurityType,
                            masterKeyCryptoData.iv
                        );
                } else {
                    cipher = BiometricCipherHelper.getCypherForEncryption(this.vault.config.key, vault.config.deviceSecurityType);
                }
            ...
            if (vaultError instanceof InvalidatedCredentialsError) {
                try {
                    this.vault.clear();
                    BiometricCipherHelper.removeKey(this.vault.config.key);
                } catch (VaultError ex) {
                    ex.printStackTrace();
                }
            }
