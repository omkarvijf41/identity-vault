...
public class VaultPlugin extends CordovaPlugin {
    ...
    private CallbackContext errorCommandCallback;
    ...
    void onError(JSONArray args, CallbackContext callbackContext) {
        this.errorCommandCallback = callbackContext;
    }
    ...
    private IdentityVaultConfig getConfig(JSONArray args) throws InvalidArgumentsError {
        try {
            ...
            if (theMessage.contains("No value for ")) {
                throw new InvalidArgumentsError(String.format("Config object is missing the required '%s' property. [Docs: https://ionic.io/docs/identity-vault/interfaces/identityvaultconfig]", theMessage.replace("No value for ", "")));
            } else {
    ...
    private void sendErrorResult(final CallbackContext callbackContext, VaultError error) {
        ...
        if (!(error instanceof MissingBiometricsError)) {
            if (this.errorCommandCallback != null) {
                this.errorCommandCallback.sendPluginResult(result);
            }
        }
    }
