...
public class IdentityVault implements LifecycleObserver {
    ...
    IdentityVault(Activity activity, JSONObject options) throws VaultError {
        ...
        activity.runOnUiThread(() -> ProcessLifecycleOwner.get().getLifecycle().addObserver(IdentityVault.this));
    }
    ...
    public void setBiometricsEnabled(boolean enabled) throws VaultError {
        if (!isBiometricsAvailable() && enabled) {
    ...
    public void setHideScreenOnBackground(boolean enabled) throws VaultError {
        config.appConfig.hideScreenOnBackground = enabled;
        sendConfigEvent();
    }
    ...
    public void onCreate() {
        // Check if app was using inMemory only and lock if so
        if (mVault.isLocked() && !mVault.isPasscodeEnabled() && !mVault.isBiometricsEnabled()) {
            Log.i(TAG, "App was quit and in memory mode is enabled. Clearing...");

            try {
                clear();
            } catch (VaultError e) {
                Log.e(TAG, "Vault error on clearing: " + e.getMessage());
            }
        }
    }
