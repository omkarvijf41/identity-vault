...
public class IonicCombinedVault {
    ...
    public void setBiometricsEnabled(boolean enabled) throws VaultError {
        ...
        if (enabled == isBiometricsEnabled()) {
            return;
        }
    ...
    public void setSecureStorageModeEnabled(boolean enabled) throws VaultError {
        ...
        if (enabled == isSecureStorageModeEnabled()) {
            return;
        }
        if (enabled) {
    ...
    private void autoGenerateKeyIfNeeded() {
        if (mStorageVault.isKeyAvailable()) {
            return;
        } // don't if the key is ready
        if (mState.inUse && !mState.isSecureStorageModeEnabled()) {
            return;
        } // don't generate if inUse (force user to clear)
    ...
    private void storeKeyInBioVault(SecretKey key) {
        if (isBiometricsEnabled()) {
            ...
            mBiometricVault.rekeyStorage(null);
            mBiometricVault.edit().putString(STORAGE_KEY, encodedKey).putString(STORAGE_KEY_ALGORITHM, key.getAlgorithm()).apply();
        }
    }
