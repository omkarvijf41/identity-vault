...
const frameworkDelegate = require('./framework-delegate-dcb7b1f5.js');
const index$1 = require('./index-f14ae831.js');
  ...
  componentDidLoad() {
    {
      rIC(async () => {
        ...
        if (!ionicGlobal.config.getBoolean('_testing')) {
          Promise.resolve().then(function () { return require('./tap-click-259474eb.js'); }).then(module => module.startTapClick(ionicGlobal.config));
        }
        if (ionicGlobal.config.getBoolean('statusTap', isHybrid)) {
          Promise.resolve().then(function () { return require('./status-tap-bbe990a0.js'); }).then(module => module.startStatusTap());
        }
        if (ionicGlobal.config.getBoolean('inputShims', needInputShims())) {
          Promise.resolve().then(function () { return require('./input-shims-ee391b3e.js'); }).then(module => module.startInputShims(ionicGlobal.config));
        }
        ...
        Promise.resolve().then(function () { return require('./focus-visible-25193389.js'); }).then(module => this.focusVisible = module.startFocusVisible());
      });
    }
  }
  /**
...
let RouterOutlet = class {
  ...
  async connectedCallback() {
    ...
    this.gesture = (await Promise.resolve().then(function () { return require('./swipe-back-50c394cc.js'); })).createSwipeBackGesture(this.el, () => !this.gestureOrAnimationInProgress && !!this.swipeHandler && this.swipeHandler.canStart(), () => onStart(), step => this.ani && this.ani.progressStep(step), (shouldComplete, step, dur) => {
      if (this.ani) {
        this.ani.onFinish(() => {
          this.gestureOrAnimationInProgress = false;
          if (this.swipeHandler) {
